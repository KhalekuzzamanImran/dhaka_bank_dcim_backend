...
def _normalize_delimiter(delimiter: str | None) -> str:
    if not delimiter:
        return ","
    raw_delimit = str(delimiter)
    clean_delimit = raw_delimit.strip().lower()
    if raw_delimit == "\t" or clean_delimit in ("\\t", "tab"):
        return "\t"
    if raw_delimit.strip() in (",", ";", "|", ":"):
        return raw_delimit.strip()
    return ","
...
def _metadata_lines(context: GeneratorContext, dataset: ReportDataset) -> list[str]:
    ...
    if _as_bool(header_config.get("show_timezone"), False) or _as_bool(footer_config.get("show_timezone"), False):
